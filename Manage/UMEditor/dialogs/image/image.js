...
(function () {
    ...
    OnlineImage.prototype = {
        ...
        pushData: function (list) {
            var i, item, img, icon, delbut, defbut, guid, _this = this,
            ...
            for (i = 0; i < list.length; i++) {
                if (list[i] && list[i].url) {
                    ...
                    defbut = document.createElement('span');
                    ...
                    img.width = 113;
                    //列表的缩略图
                    img.setAttribute('src', urlPrefix + list[i].smallurl + (list[i].smallurl.indexOf('?') == -1 ? '?noCache=' : '&noCache=') + (+new Date()).toString(36));
                    //用于插入到页面的大图路径
                    img.setAttribute('_src', urlPrefix + list[i].url);

                    domUtils.addClass(icon, 'icon');
                    domUtils.addClass(delbut, 'delbut');
                    if (list[i].sort == 0)
                        domUtils.addClass(defbut, 'defbut def');
                    else {
                        domUtils.addClass(defbut, 'defbut');
                        defbut.setAttribute('title', "点击设为默认图片");
                    }

                    delbut.setAttribute('id', list[i].guid);
                    delbut.setAttribute('title', "点击删除图片");
                    defbut.setAttribute('id', list[i].guid);


                    item.appendChild(img);
                    item.appendChild(icon);
                    item.appendChild(delbut);
                    item.appendChild(defbut);

                    //删除图片
                    domUtils.on(delbut, 'click', function () {
                        if (confirm("确认要删除此图片吗？")) {
                            var delurl = editor.getActionUrl(editor.getOpt('imageManagerDelActionName')),
                            isJsonp = utils.isCrossDomainUrl(delurl);
                    ...
                    domUtils.on(defbut, 'click', function () {
                        var setstate = 0;
                        if (this.getAttribute("class") == "defbut def") {
                            setstate = 1;
                        }
                        var url = editor.getActionUrl(editor.getOpt('imageManagerSetDefActionName')),
                            isJsonp = utils.isCrossDomainUrl(url);
                        var li = this;
                        ajax.request(url, {
                            'timeout': 100000,
